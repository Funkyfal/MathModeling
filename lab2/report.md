...
2. **Оценки по выборке**
   ...
   * Несмещённая оценка дисперсии:
     ...
     $$
   * Сравнить $\bar X$ и $s^2$ с теоретическими $E[X]$ и $\operatorname{Var}(X)$.
...
```java
// Lab2_Discrete.java
// Лабораторная №2 — моделирование дискретных СВ (вариант 2)
//  - Bernoulli Bi(1,p), p=0.5
//  - Negative Binomial (r=5, p=0.25) (число неудач до r-й успеха)
// Выполняет: генерацию n реализаций, оценки mean/var, χ^2 тест и оценку ошибки I рода
...
1. **Сравнение моментов.** Для Бернулли выборочное среднее и дисперсия близки к теоретическим значениям (например, $\bar X = 0.503,\ s^2 = 0.249$ при теории $E=0.5,\ \operatorname{Var}=0.25$). Для отрицательного биномиала наблюдаются близкие значения (пример: $\bar X = 15.12,\ s^2 = 59.1$ против теории $E=15,\ \operatorname{Var}=60$).
